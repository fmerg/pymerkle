...
@pytest.mark.parametrize("num, expected", [(nums[i], logarithms[i]) for i in range(len(logarithms))])
...
def reverseTupleFromList(_list):
    """Helper function for the next test session. Returns tuple from list in reverse order
    """
...
@pytest.mark.parametrize( "num, powers", [(nums[i], mixed_powers[i]) for i in range( len(nums))])
