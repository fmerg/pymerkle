...
_*records_, string or bytes-like objects indifferently, thought of as the records encrypted into the Merkle-tree upon construction; usually _not_ provided

*hash_type*, string indicating the hash algorithm used by the Merkle-tree. Can be any of the following:

`'md5'`, `'sha224'`, `'sha256'`, `'sha384'`, `'sha512'`

(upper- or mixed-case allowed); if `sha3` is moreover supported, can also be

`'sha3_224'`, `'sha3_256'`, `'sha3_384'`, `'sha3_512'`

(upper- or mixed-case with '-' instead of '_' allowed)

*encoding_type*, string indicating the encoding used by the Merkle-tree before hashing. Can be any of the following:
